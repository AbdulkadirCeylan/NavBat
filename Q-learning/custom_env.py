...
class QuadCopterEnv(gym.Env):
    ...
    def reset(self):
        ...
        observation = [self.area,self.x_center]
        return observation
    ...
    def step(self, action):
        ...
        while self.distance > 0.15:
            self.distance,theta =  self.calculate_target_distance()
            if(time.time()-t1) >10:
                break
    ...
    def process_data(self, data_position, data_imu,dst,eulerr):
        ...
        reward = 0
        if data_position[0] > 13 or data_position[0] <-1.5 or data_position[1] > 7 or data_position[1] < -7:
            done = True
            print("Out of area")
        ...
        if self.area > self.previous_area and (self.x_center < 34 and self.x_center >16):
            reward -= 5

        if not (self.x_center < 34 and self.x_center >16):
            reward = +5
